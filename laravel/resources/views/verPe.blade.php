   ...
   <div class="px-6 mb-10">
    <div class="flex items-center gap-3">
     ...
     <div>
      ...
      <p class="text-[0.65rem] uppercase tracking-[0.2em] text-secondary">Precision Control</p>
     </div>
    </div>
   </div>
  ...
  <main class="ml-64 min-h-screen bg-surface flex flex-col">
   <header class="h-24 px-10 flex items-center justify-between bg-surface-container-low">
    <div class="flex flex-col">
     <h2 class="text-3xl font-black tracking-tight text-on-surface uppercase">Ver pedidos</h2>
    </div>
   ...
   <section class="grid grid-cols-4 gap-0 border-b border-outline-variant/10">
    <div class="p-8 border-r border-outline-variant/10 bg-surface">
     <p id="kpi-total" class="text-4xl font-black tracking-tighter text-on-surface">0</p>
    </div>
    <div class="p-8 border-r border-outline-variant/10 bg-surface">
     <p id="kpi-pendientes" class="text-4xl font-black tracking-tighter text-on-primary-container">0</p>
    </div>
    <div class="p-8 border-r border-outline-variant/10 bg-surface">
     <p id="kpi-activos" class="text-4xl font-black tracking-tighter text-on-surface">0</p>
    </div>
    <div class="p-8 bg-surface">
     <p id="kpi-efectividad" class="text-4xl font-black tracking-tighter text-primary">0%</p>
    </div>
   </section>
     ...
     <table class="w-full text-left border-collapse">
      <thead>
       <tr class="bg-surface-container-highest">
        <th class="px-6 py-4 text-[0.65rem] uppercase font-black tracking-[0.2em] text-secondary">Order ID</th>
        <th class="px-6 py-4 text-[0.65rem] uppercase font-black tracking-[0.2em] text-secondary">Client / Entity</th>
        <th class="px-6 py-4 text-[0.65rem] uppercase font-black tracking-[0.2em] text-secondary">Priority</th>
        <th class="px-6 py-4 text-[0.65rem] uppercase font-black tracking-[0.2em] text-secondary">Status</th>
        <th class="px-6 py-4 text-[0.65rem] uppercase font-black tracking-[0.2em] text-secondary text-center">Actions</th>
       </tr>
      </thead>
      <tbody class="divide-y divide-outline-variant/5" id="pedidos_admin_grid"></tbody>
     </table>
    </div>
   </section>
  </main>
  <script>
   ...
   document.addEventListener("DOMContentLoaded", async () => {
       try {
           ...
           const pendientes = window.all_pedidos.filter(p => p.estatus === 'Pendiente').length;
           const activos = window.all_pedidos.filter(p => p.estatus !== 'Cancelado').length;
           const efectividad = total > 0 ? Math.round((activos / total) * 100) : 0;
           
           document.getElementById('kpi-total').innerText = total.toLocaleString();
           document.getElementById('kpi-pendientes').innerText = pendientes.toLocaleString();
           document.getElementById('kpi-activos').innerText = activos.toLocaleString();
           document.getElementById('kpi-efectividad').innerText = efectividad + '%';
